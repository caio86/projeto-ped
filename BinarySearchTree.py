...
class BinarySearchTree:
    ...
    def remove_duplicates(self):
        self.__root = self.__remove_duplicates(self.__root)

    def __remove_duplicates(self, node:Node):
        if node is None:
            return None

        node.left = self.__remove_duplicates(node.left)
        node.right = self.__remove_duplicates(node.right)
